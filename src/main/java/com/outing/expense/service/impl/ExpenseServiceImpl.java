...
//import com.outing.expense.api.exception.ExpenseManagerException;
...
public class ExpenseServiceImpl implements ExpenseService {
    ...
    public ExpenseDto addExpense(ExpenseDto expenseDto) {
        ...
        newExpenseDto.setOutingId(expenseDto.getOutingId());
        ExpenseModel expenseModel = new ExpenseModel(expenseId, date,totalAmount,description,false,loggedInUserId,expenseDto.getOutingId());
    ...
    public ExpenseDto updateExpense(String expenseId, ExpenseDto expenseDto){
        ...
        ExpenseShareDto[] expenseShareDto = expenseDto.getExpenseDetails();
        for (ExpenseShareDto shareDto : expenseShareDto) {
            friendsId.add(shareDto.getUserId());
        }
        List<FriendshipDto> friends = friendshipCheckController.getAllFriendsByUserId(loggedInUserId).getBody();
        for(String friendId:friendsId){
            if(isNotFriendWithCurrentLoggedInUser(friends, friendId)){
                throw new OutingException("User is not friend with id "+friendId,HttpStatus.FORBIDDEN);
            }
        }
        ExpenseModel expenseModel = getExpenseById(expenseId);
        List<ExpenseShareModel> expenseShareModels = getExpenseShareByExpenseId(expenseId);
        int totalAmount = expenseDto.getTotalAmount();
        String description = expenseDto.getDescription();
        if(description==null || description.isEmpty()){
            expenseDto.setDescription("Outing");
        ...
        String date = expenseDto.getDate();
        if(date==null || date.isEmpty())date = expenseModel.getDate();
        else date = expenseDto.getDate();
        int totPaidAmount = 0;
        int totSharAmount = 0;
        List<String> friendIds = new ArrayList<>();
        List<Integer> expenseShares = new ArrayList<>();
        List<Integer> paidAmounts = new ArrayList<>();
        List<ExpenseShareModel> newExpenseShareModels = new ArrayList<>();
        Map<String,Boolean> check = new HashMap<>();
        for(int i = 0; i< expenseDto.getExpenseDetails().length; i++) {
            if (expenseShareRepository.existsByUserIdAndExpenseId(expenseDto.getExpenseDetails()[i].getUserId(),expenseId)) {
                ExpenseShareModel existingExpenseShareModel = expenseShareRepository.findByUserIdAndExpenseId(expenseDto.getExpenseDetails()[i].getUserId(),expenseId);
                friendIds.add(expenseDto.getExpenseDetails()[i].getUserId());
                expenseShares.add(expenseDto.getExpenseDetails()[i].getBeneficiaryAmount());
                paidAmounts.add(expenseDto.getExpenseDetails()[i].getBearerAmount());
                check.put(expenseDto.getExpenseDetails()[i].getUserId(),true);
                expenseDto.getExpenseDetails()[i].setId(existingExpenseShareModel.getId());
            } else {
                String expenseShareId = UUID.randomUUID().toString();
                ExpenseShareModel newExpenseShareModel = new ExpenseShareModel(expenseShareId, expenseId, expenseDto.getExpenseDetails()[i].getUserId(), expenseDto.getExpenseDetails()[i].getBearerAmount(), expenseDto.getExpenseDetails()[i].getBeneficiaryAmount(),"invited", expenseDto.getExpenseDetails()[i].getName());
                expenseDto.getExpenseDetails()[i].setId(expenseShareId);
                newExpenseShareModels.add(newExpenseShareModel);
            }
            expenseDto.getExpenseDetails()[i].setExpenseId(expenseId);
            totPaidAmount+= expenseDto.getExpenseDetails()[i].getBearerAmount();
            totSharAmount+= expenseDto.getExpenseDetails()[i].getBeneficiaryAmount();
        }
        errors = amountCheck(totalAmount, totPaidAmount, totSharAmount);
        if(!errors.isEmpty()){
            throw new OutingException(errors, HttpStatus.FORBIDDEN);
        ...
        newExpenseDto.setDate(date);
        newExpenseDto.setExpenseDetails(expenseDto.getExpenseDetails());
        newExpenseDto.setCreatorId(expenseDto.getCreatorId());
        expenseRepository.save(new ExpenseModel(expenseId,date, expenseDto.getTotalAmount(), expenseDto.getDescription(),false,expenseModel.getCreatorId(),expenseModel.getOutingId()));
        for(int i=0;i< friendIds.size();i++){
            expenseShareRepository.updateFriendExpense(friendIds.get(i), expenseId, expenseShares.get(i), paidAmounts.get(i));

        }
        for(ExpenseShareModel expenseShareModel:expenseShareModels){
            if(!check.containsKey(expenseShareModel.getUserId())){
                expenseShareRepository.deleteByUserIdAndExpenseId(expenseShareModel.getUserId(),expenseId);
            }
        }
        expenseShareRepository.saveAll(newExpenseShareModels);
        return newExpenseDto;
    }
    ...
    public void deleteExpenseById(String expenseId) {
        ExpenseModel expenseModel = expenseRepository.findById(expenseId).orElseThrow(()->new OutingException(Constants.INVALID_EXPENSE,HttpStatus.NOT_FOUND));
        expenseModel.setDeletedStatus(true);
        expenseRepository.save(expenseModel);
    }
    ...
    public List<ExpenseDto> getAllExpensesByUserId(Integer pageNumber,Integer pageSize) {
        ...
        List<ExpenseShareModel> expenseShareModels =  expenseShareRepository.findByUserIdAndExpenseNotDeleted(loggedInUserId,pageable);
...
//            Arrays.sort(expenseShareDto, Comparator.comparing(dto -> dto.getStatus()));
            ...
            ExpenseDto expenseDto = new ExpenseDto(expenseModel.getId(),expenseModel.getDescription(),expenseModel.getTotalAmount(),expenseModel.getDate(),expenseModel.getCreatorId(),expenseShareDto, expenseModel.getOutingId());
            expenseDtos.add(expenseDto);
        }

        expenseDtos.sort(Comparator.comparing(ExpenseDto::getDate).reversed());
        return expenseDtos;
    }
    ...
    public List<ExpenseDto> getAllOutingExpensesByUserId(String outingId, Integer pageNumber,Integer pageSize) {
        String loggedInUserId = principalDetails.getPrincipalDetails().getId();
        Pageable pageable = PageRequest.of(pageNumber, pageSize);
        List<ExpenseShareModel> expenseShareModels =  expenseShareRepository.findByUserIdAndExpenseNotDeleted(loggedInUserId, outingId);
        System.out.println(expenseShareModels);
        List<ExpenseDto> expenseDtos = new ArrayList<>();
        int sz = 0;
        for(ExpenseShareModel expenseShareModel:expenseShareModels){
...
//            Arrays.sort(expenseShareDto, Comparator.comparing(dto -> dto.getStatus()));
    ...
    public ExpenseDto approveExpense(Map<String, String> approvalRequest) {
        String loggedInUserId = principalDetails.getPrincipalDetails().getId();
        String expenseId = approvalRequest.get("expenseId");
        String userId = approvalRequest.get("userId");
        String approvalResponse = approvalRequest.get("acceptanceStatus");
        if(!Objects.equals(userId,loggedInUserId)){
            throw new OutingException("You are not authorized to approve this",HttpStatus.FORBIDDEN);
        }
        ExpenseModel expenseModel = getExpenseById(expenseId);
        ExpenseShareModel expenseShareModel = expenseShareRepository.findByUserIdAndExpenseId(userId,expenseId);
//        if(Objects.equals(expenseShareModel.getStatus(), "accepted")){
//            throw new OutingException("User has already Acknowledged",HttpStatus.FORBIDDEN);
//        }
        expenseShareModel.setStatus(approvalResponse);
        expenseShareRepository.save(expenseShareModel);
        List<ExpenseShareModel> expenseShareModels = getExpenseShareByExpenseId(expenseId);
        ExpenseShareDto[] newExpenseShareDto = new ExpenseShareDto[expenseShareModels.size()];
        for(int i=0;i< expenseShareModels.size();i++){
            newExpenseShareDto[i] = new ExpenseShareDto();
            newExpenseShareDto[i].setId(expenseShareModels.get(i).getId());
            newExpenseShareDto[i].setUserId(expenseShareModels.get(i).getUserId());
            newExpenseShareDto[i].setExpenseId(expenseShareModels.get(i).getExpenseId());
            newExpenseShareDto[i].setName(expenseShareModels.get(i).getName());
            newExpenseShareDto[i].setStatus(expenseShareModels.get(i).getStatus());
            newExpenseShareDto[i].setBearerAmount(expenseShareModels.get(i).getBearerAmount());
            newExpenseShareDto[i].setBeneficiaryAmount(expenseShareModels.get(i).getBeneficiaryAmount());
        }
//        Arrays.sort(newExpenseShareDto, Comparator.comparing(ExpenseShareDto::getStatus));
        Arrays.sort(newExpenseShareDto, (dto1, dto2) -> {
            // Custom comparator to prioritize "invited" > "rejected" > "accepted"
            String status1 = dto1.getStatus();
            String status2 = dto2.getStatus();
        ...
        ExpenseDto newExpenseDto = new ExpenseDto(expenseModel.getId(),expenseModel.getDescription(),expenseModel.getTotalAmount(),expenseModel.getDate(),expenseModel.getCreatorId(),newExpenseShareDto);
        return newExpenseDto;
    }
}
